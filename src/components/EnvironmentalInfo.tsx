...
export default function EnvironmentalInfo() {
  ...
  // Mock weather data for 3 days
  const weather = [
    { day: '今天', condition: '晴朗', temp: 24, humidity: 45, Icon: Sun, iconColor: 'text-amber-400' },
    { day: '明天', condition: '多云', temp: 22, humidity: 55, Icon: Cloud, iconColor: 'text-slate-400' },
    { day: '后天', condition: '小雨', temp: 19, humidity: 80, Icon: CloudRain, iconColor: 'text-blue-400' },
  ];
    ...
    <div className="glass-card h-full flex flex-col">
      ...
      <div className="panel-content flex-1 flex flex-col gap-1.5 overflow-y-auto">
            ...
            <div key={idx} className={`${idx === 0 ? 'bg-sky-900/10 border-sky-800/30' : 'bg-slate-800/10 border-slate-700/30'} px-2 py-1 rounded border transition-colors`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1">
                  <item.Icon className={`${item.iconColor} flex-shrink-0`} style={{ width: 'clamp(16px, 1.5vw, 28px)', height: 'clamp(16px, 1.5vw, 28px)' }} />
                  <div className={`text-[clamp(10px,0.85vw,14px)] uppercase font-bold tracking-tighter ${item.iconColor}`}>{item.condition}</div>
                </div>
        ...
        <div className="shrink-0 flex justify-between items-center text-[clamp(10px,0.85vw,14px)] px-1 pt-1">
          <span className="text-slate-400">雨雪传感器: <span className="text-emerald-400">无 雨雪</span></span>
        </div>
        ...
        <div className="shrink-0 flex flex-col h-[125px]">
          <div className="flex justify-between items-center text-[clamp(10px,0.85vw,14px)] mb-1 px-1">
            ...
            <span className="text-sky-400 font-mono">AVG: 60%</span>
          </div>
            ...
            <div className="flex-1 px-2 py-2">
              <div className="grid grid-cols-2 gap-2 text-[clamp(10px,0.85vw,14px)]">
                <div className="flex items-center gap-1">
                   ...
                   <span className="text-slate-200 font-bold">3天</span>
                </div>
                <div className="flex items-center gap-1">
                   <span className="text-slate-400">实测有雪累计</span>
                   <span className="text-slate-200 font-bold">5次</span>
                </div>
                <div className="flex items-center gap-1">
                   <span className="text-slate-400">执行融雪测试</span>
                   <span className="text-slate-200 font-bold">3次</span>
                </div>
                <div className="flex items-center gap-1">
